                            ...
                            <div class="row">
                                    ...
                                    <div class="form-group product-add">
                                        ...
                                        <select class="product-create" name="brand_id"
                                            ...
                                            <option value="{{ $brand->id }}"
                                                @selector((old('brand')?old('brand'):$product->brand_id),$brand->id)>
                                                {{ $brand->name }}
                                            </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group product-add">
                                        ...
                                        <input value="{{ $product->name }}" onfocus="hideValidation(this.name)"
                                            type="text" name="name" class="product-create"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group product-add">
                                        ...
                                        <input value="{{ $product->actual_price }}" onfocus="hideValidation(this.name)"
                                            onkeyup="updateFixedPrice()" type="number" min="0" name="actual_price"
                                            id="actual_price" class="product-create" placeholder="Enter product price">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group product-add">
                                        ...
                                        <input value="{{ $product->discount }}" onfocus="hideValidation(this.name)"
                                            onkeyup="updateFixedPrice()" type="number" min="0" max="100" maxlength="3"
                                            name="discount" id="discount" class="product-create"
                                ...
                                <div class="col-md-6">
                                    <div class="form-group product-add">
                                        ...
                                        <input value="{{ $product->fixed_price }}" onfocus="hideValidation(this.name)"
                                            type="number" name="fixed_price" id="fixed_price" class="product-create"
                                    ...
                                    <div class="form-group product-add">
                                        ...
                                        <div>
                                            <x-image-picker type="media"
                                                preload-media="{!! $product->preload_media !!}" />
                                        </div>
...
<script>
    ...
    //server data contained variables
    var category_id={!! json_encode($product->parents_path) !!},
        server_available_color={!! json_encode($product->available_color) !!},
        server_available_size={!! json_encode($product->available_size) !!},
        server_variants={!! json_encode($product->variants) !!},
        server_highlight={!! json_encode($product->highlight) !!},
        server_specification={!! json_encode($product->specification) !!};
    //local variables
    var active_ids = [],
        active_menu = [],
        available_size = [],
            ...
            final_html = `
                <li class="nav-item mb-1 d-flex justify-content-start align-items-center">
                    <div onclick='selectCategory(` + JSON.stringify(dataObj) + `,${ current_cursor })'
                        class="nav-link w-100" id="menu-select">
    ...
    function addHighlights(value='') {
        ...
        if (loc_validation) {
            ...
            <div class="row">
                ...
                <div class="col-11">
                    <input value="${value}" id='highlight-${ highlight }' data-index="${ highlight }" onfocus="hideValidation('highlight')" type="text" name="highlight[]" class="product-create m-2"
    ...
    var specification = 1;
    function addSpecifications(val={title:'',content:''}) {
        if (specification > 10) {
        ...
        if (loc_validation) {
            ...
            <div class="row">
                ...
                <div class="col-3 p-0">
                    <input value="${val.title}" id='specification-title-${ specification }' data-index="${ specification }" onfocus="hideValidation('specification')" type="text" name="specification-title[]" class="product-create m-2"
                    placeholder="Specification title">
                </div>
                <div class="col-8">
                    <input value="${val.content}" id='specification-${ specification }' data-index="${ specification }" onfocus="hideValidation('specification')"
    ...
    function validateProductForm() {
        ...
        let choosed_media=media_api.getChoosedFiles();
        let media=media_api.getFiles();
        let files=media.filter(ele=>ele.format=="image").map(ele=>ele.editor.crop);validation_flag=true
        if (media.length==0 && choosed_media.length==0) validation_error.push({id:'media',message:"Please select your product Images & Videos"});
        else{
            choosed_media.forEach((ele,ind)=>{
        ...
        if (validation_error.length == 0 && hide_validation_error.length == 13) {
            ...
            console.log(validation_error , hide_validation_error.length)
        }
    }
    ...
    function sendRequest() {
        ...
        fetch("{{ route('vendor.products.update',[$product->id]) }}", options)
            .then(response => response.json())
            .then(res => {
                One.loader('hide')
                if (res.status) {
                    window.location = '{{ route("vendor.products.index",["status"=>"success","message"=>"Product Edited successfully"]) }}'
                } else {
    ...
    function loadServerData(){
        category_id.reverse().forEach((ele,ind)=>{
            jQuery('#menu-'+ele.id).click()
            if((category_id.length-1)==ind) jQuery('#menu-select').click()
        });
        server_available_size.forEach(ele=>{
            var product_size = jQuery('#product_size');
            product_size.val(ele)
            addOptions()
        })
        server_available_color.forEach(ele=>{
            var color_code = jQuery('#color_code');
            var color_name = jQuery('#color_name');
            color_code.val(ele.code);
            color_name.val(ele.name)
            addOptions()
        })
        variants.forEach((ele,ind)=>{
            let index=server_variants.findIndex(ele_2=>ele_2.size==ele.size && ele_2.color==`${ele.color.code}||${ele.color.name}`)
            if(index===-1){
                jQuery("#checkbox-"+ele.id).click();
            }else{
                let s_q=server_variants[index].quantity
                jQuery("#quantity-"+ele.id).val(s_q);
                variants[index].quantity=s_q;
            }
        })
        updateQuantity()
        server_highlight.forEach(ele=>addHighlights(ele))
        server_specification.forEach(ele=>addSpecifications(ele))
    }
