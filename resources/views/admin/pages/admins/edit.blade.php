...
<div class="bg-body-light">
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <h1 class="flex-sm-fill h3 my-2">Edit Admin</h1>
...
<div class="content">
    ...
    <div class="block block-rounded">
        <div class="block-content block-content-full vendors-frm">
            <form class="js-validation" action="{{ route('admin.admins.update',[$admins->id]) }}" method="post">
                @method('put')
                @csrf
                <div class="row">
                    <div class="col-md-6 form-group">
                        <label>Name</label>
                        <input type="text" class="add-admin-frm" name="name" id="name"
                            value="{{ old('name')?old('name'):$admins->name }}">
                        @error('name')
                    ...
                    {{-- <div class="col-md-4 form-group">
                        ...
                        <input type="text" class="add-admin-frm" name="username" id="username"
                            value="{{ old('username')?old('username'):$admins->username }}">
                        @error('username')
                        <div class="text-danger animated fadeIn">
                    ...
                    <div class="col-md-6 form-group">
                        <label>E-mail</label>
                        <input type="email" class="add-admin-frm" name="email" id="email"
                            value="{{ old('email')?old('email'):$admins->email }}">
                        @error('email')
                        <div class="text-danger animated fadeIn">
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <select class="add-admin-frm add-admin-frms" name="roles[]">
                            ...
                            <option value="{{ $role->id }}" @selector((old('roles')?old('roles')[0]:$role->
                                id),strval($admins->roles->implode('id')))
                                >{{ $role->name }}
                            </option>
                            @endforeach
                        </select>
