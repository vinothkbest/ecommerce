...
<div class="bg-body-light">
    <div class="content content-full">
        <div class="d-flex flex-column flex-sm-row justify-content-sm-between align-items-sm-center">
            <h1 class="flex-sm-fill h3 my-2">Edit Enquiry</h1>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <select class="add-admin-frm add-admin-frms" name="user_id">
                                ...
                                <option value="{{ $user->id }}" @selector((old('user_id')?old('user_id'):$enquiry->
                                    user_id),$user->id)>
                                    {{ "YRUC".$user->id." - ".$user->name }}
                                </option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="add-admin-frm add-admin-frms" name="product_id">
                                ...
                                <option value="{{ $product->id }}"
                                    @selector((old('product_id')?old('product_id'):$enquiry->
                                    product_id),$product->id)>{{ "YRPC".$product->id." - ".$product->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="add-admin-frm add-admin-frms" name="status">
                                <option @selector((old('status')?old('status'):$enquiry->status),'1') value="1">Active
                                </option>
                                <option @selector((old('status')?old('status'):$enquiry->status),'0') value="0">Disabled
                                </option>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <textarea class="add-admin-frm" name="description" rows="4"
                                placeholder="description content..">{{ old('description')?old('description'):$enquiry->description }}</textarea>
                        </div>
